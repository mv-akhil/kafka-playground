...
public class ConsumerAutoOffsetCommit {
    ...
    private static final Logger log = LoggerFactory.getLogger(ConsumerAutoOffsetCommit.class.getSimpleName());
    public static void main(String[] args) {
        ...
        try{
            ...
            while(true)
            {
                ...
                // Whenever we do poll, if the list offset commit exceeds the auto commit interval, new offset commit async request is sent
                // In this scenario we are in atleast once read scenario, because we are polling the messages only after processing
                // In case we are processing in async and if the processing fails,
                // poll is happening in parallel and offset is committed where messages are not processed
